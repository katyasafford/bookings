...
// returns true if room id is availalbe for the specified dates, and false otherwise
...
// returns a slice of available rooms for a given date range, if any
func (m *postgresDBRepo) SearchAvailalbilityForAllRooms(start, end time.Time) ([]models.Room, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
...
// returns a slice of all reserrvations
...
// returns a slice of new reserrvations
